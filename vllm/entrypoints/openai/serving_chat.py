...
class OpenAIServingChat(OpenAIServing):
        ...
        if self.enable_reasoning:
            try:
                ...
                raise TypeError(
                    ...
                    f"--reasoning-parser '{reasoning_parser}' which has not "
                    "been registered") from e
    ...
    async def chat_completion_stream_generator(
        ...
        if tool_choice_auto or should_stream_with_reasoning:
            # Required for tool-auto parsing and reasoning parsing.
        ...
        # Prepare the reasoning parser if it's needed
        try:
            if should_stream_with_reasoning and self.reasoning_parser:
                reasoning_parsers: List[Optional[ReasoningParser]] = [
        ...
        try:
            async for res in result_generator:
                ...
                for output in res.outputs:
                    ...
                    # handle streaming deltas with reasoning parser
                    elif should_stream_with_reasoning:
                        assert previous_texts is not None
                        assert all_previous_token_ids is not None
    ...
    def _should_stream_with_reasoning_parsing(
            self, request: ChatCompletionRequest) -> bool:
